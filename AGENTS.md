...
## Operations
- **Run:** `uv run main.py <repo_url>` (CLI entry point)
...
## Architecture & Style
- **Structure:** Source in `src/` (graph, nodes, state, utils, logger). Entry in `main.py`.
...
- **Error Handling:** Typed exceptions. No bare `except:`. Log errors with `logger`.
- **Async:** Use `asyncio` for concurrent LLM calls or I/O.
- **Security:** Never commit secrets/keys. Use env vars for API keys.
- **Logging:** Use `logger` from `src.logger` for consistent logging.
- **Docs:** Keep `README.md` updated. Use docstrings for functions/classes.
